...
public class FIDOAuthenticator extends AbstractApplicationAuthenticator
    ...
    protected void processPasskeyEnrollmentResponse(HttpServletRequest request, HttpServletResponse response,
        ...
        if (challengeResponse != null && !challengeResponse.contains(ERROR_CODE)) {
            ...
            // Set the key name
            setPasskeyDisplayName(credentialId, displayName, user.getUsernameAsSubjectIdentifier(true, true), context);
    ...
    private void handleUnProvisionedFederatedUser(HttpServletResponse response) throws AuthenticationFailedException {
        ...
        try {
            response.sendRedirect(getProvisionedUserNotFoundRedirectUrl());
    ...
    private String getProvisionedUserNotFoundRedirectUrl()
            throws UnsupportedEncodingException, URLBuilderException, URISyntaxException {

        String provisionedUserNotFoundRedirectUrl = ConfigurationFacade.getInstance().getAuthenticationEndpointURL()
                .replace(FIDOAuthenticatorConstants.URI_LOGIN, FIDOAuthenticatorConstants.URI_ERROR);

        provisionedUserNotFoundRedirectUrl =
                provisionedUserNotFoundRedirectUrl + ("?") + "&authFailure=true&authFailureMsg=" +
                        URLEncoder.encode(
